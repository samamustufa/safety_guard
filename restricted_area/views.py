...
def process_restricted_area(request):
    try:
        ...
        data = request.POST.get('coordinates')
        ...
        # Store restricted area data
        store_restricted_area_data(video_path, person_count)
...
def restricted_area_result(request):
    try:
        # Retrieve the person count from the JSON response
